          ...
          {step === 'prerequisites' && (
            <div className="flex flex-col items-center text-center gap-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Checking for required CLI tools</p>
              </div>
                ...
                {prerequisitesLoading || !prerequisites ? (
                  ...
                  <>
                    <PrereqRow
                      ...
                      detail={prerequisites.claude_code ? 'installed' : 'not found'}
                      required
                    />
                    <PrereqRow
                      label="GitHub CLI (gh)"
                      installed={prerequisites.github_cli}
                      detail={prerequisites.github_cli ? 'authenticated' : 'not found'}
                    />
                    <PrereqRow
                      label="Codex CLI"
                      installed={prerequisites.codex}
                      detail={prerequisites.codex ? 'installed' : 'not found'}
                    />

                    <p className="pt-2 text-xs text-slate-500 leading-relaxed">
                      Claude Code is required. GitHub CLI and Codex are optional but unlock more
                      features.
                    </p>
                  </>
                )}
                  ...
                  >
                    <div
                      className={`flex h-5 w-5 items-center justify-center rounded-full border-2 transition-colors ${
                        selectedModel === opt.value ? 'border-amber-400' : 'border-slate-600'
                      }`}
                    >
                      {selectedModel === opt.value && (
            ...
            <div className="flex flex-col items-center text-center gap-6">
              ...
              <div className="w-full rounded-lg border border-[#1a1a1a] bg-[#0f1117] px-4 py-3">
                <p className="text-xs text-slate-400">
                  <span className="font-medium text-slate-300">Tip:</span> Press{' '}
                  <Badge
                    variant="outline"
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-[#1a1a1a] bg-[#0f1117] px-4 py-3">
      <div className="flex items-center gap-3">
        <span className={`text-sm ${installed ? 'text-emerald-400' : 'text-slate-600'}`}>
          {installed ? '\u2713' : '\u2717'}
        </span>
      ...
      <span className={`text-xs ${installed ? 'text-emerald-400/70' : 'text-slate-600'}`}>
        {detail}
      </span>
    </div>
  );
