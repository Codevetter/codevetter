  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)] px-6 py-16 text-slate-100">
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        {mode === 'fixture' && (
          <FixtureTrack runs={runs} selected={selected} setSelectedId={setSelectedId} />
        )}
      </div>
    </div>
  );
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-slate-100">{run.label}</span>
                  </div>
                  <p className="mt-1 text-xs text-slate-500">
                    {run.fixture_id} · route {run.result.route}
                  </p>
                </div>
                <Badge variant={pass ? 'secondary' : 'destructive'}>{pass ? 'PASS' : 'FAIL'}</Badge>
              </div>
            ...
            <ol className="mt-3 space-y-2 text-sm text-slate-300">
              {selected.result.steps?.map((step) => (
                <li key={step.index} className="flex items-start gap-2">
                  ...
                  <div>
                    <span className="font-mono text-xs text-cyan-200">{step.action}</span>{' '}
              ...
              {selected.result.observations?.map((obs, idx) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <span className="font-medium text-slate-100">{obs.description}</span>
                    <Badge variant={obs.pass ? 'secondary' : 'destructive'}>
                      {obs.pass ? 'ok' : 'fail'}
                    </Badge>
                  </div>
                  <p className="mt-1 text-xs text-slate-400">{obs.detail}</p>
                  <p className="mt-1 text-xs text-slate-500 font-mono">kind: {obs.kind}</p>
                </li>
              ))}
            </ul>
          </section>
