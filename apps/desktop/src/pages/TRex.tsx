...
export default function TRex() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [w, r] = await Promise.all([listTrexWatchers(), listTrexPrRuns(undefined, 50)]);
      setWatchers(w);
      setRuns(r);
    } catch (e) {
    ...
    <div className="mx-auto max-w-6xl px-6 pb-24 pt-20">
      <header className="mb-6">
        ...
        <p className="mt-1 max-w-3xl text-sm text-[var(--text-secondary)]">
          Polls open PRs on a registered repo, runs the sandbox on any commit whose head SHA
          changed, and posts a <span className="font-mono">codevetter/t-rex</span> commit status
          back to GitHub. Uses your stored <span className="font-mono">github_token</span> (Settings
      ...
      <Card className="mb-6 border-[var(--cv-line)] bg-[var(--bg-surface)]">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            Absolute path to a local Git checkout with <span className="font-mono">gh</span> CLI
            available. Interval applies between polls (minimum 60 s).
          </CardDescription>
        </CardHeader>
      ...
      <Card className="mb-6 border-[var(--cv-line)] bg-[var(--bg-surface)]">
        <CardHeader className="pb-3 flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription className="text-xs">
              {watchers.length} registered · {watchers.filter((w) => w.enabled).length} enabled
            </CardDescription>
          </div>
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="truncate font-mono text-xs">{w.repo_path}</span>
                    </div>
                    <div className="mt-1 flex flex-wrap items-center gap-3 text-[11px] text-[var(--text-secondary)]">
                      ...
                      {w.base_branch && <span className="font-mono">base={w.base_branch}</span>}
                      {w.last_error && (
                        <span className="text-red-400">last error: {w.last_error}</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            <div className="space-y-1.5">
              {runs.map((r) => (
                ...
                >
                  ...
                  <span className="font-mono text-[var(--text-secondary)]">PR #{r.pr_number}</span>
