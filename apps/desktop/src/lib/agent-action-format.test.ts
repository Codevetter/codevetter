...
describe('describeAction', () => {
  it('renders click with selector', () => {
    assert.equal(
      describeAction(step({ type: 'click', selector: '#dl', reasoning: 'primary cta' })),
      'click #dl'
    );
  });
  ...
  it('renders done and give_up labels', () => {
    assert.equal(describeAction(step({ type: 'done', reasoning: 'found it' })), 'done');
    assert.equal(describeAction(step({ type: 'give_up', reasoning: 'stuck' })), 'gave up');
  });
});
...
describe('actionReasoning', () => {
  it('returns the reasoning on each variant', () => {
    assert.equal(
      actionReasoning(step({ type: 'scroll', delta: 600, reasoning: 'see footer' })),
      'see footer'
    );
  });
});
