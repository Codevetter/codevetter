...
export function buildCommitIntentReport(fixture: CommitIntentFixture): CommitIntentReport {
  ...
  const totalChanged = fixture.changedFiles.reduce(
  ...
  return {
    ...
    verificationGaps: [
      ...(!input.changeDescription.trim()
        ? ['Original goal/change description was not captured.']
        : []),
      ...(unchecked > 0
        ? [`${unchecked} finding${unchecked === 1 ? '' : 's'} still unchecked.`]
        : []),
      ...
      ...(reproduced > 0
        ? [
            `${reproduced} reproduced finding${reproduced === 1 ? '' : 's'} still need a fix or re-check.`,
          ]
        : []),
    ],
    evidenceSummary: [
  ...
  const timeline: ReviewTimelineItem[] = [
    {
      ...
      detail:
        input.changeDescription.trim() || input.diffRange || 'No explicit change description.',
...
function inferIntent(fixture: CommitIntentFixture, surfaces: string[]) {
  ...
  if (message.includes('review') || surfaces.includes('test'))
    return 'Improve review workflow confidence';
...
function inferReviewRisks(input: ReviewIntentInput, surfaces: string[], highRiskUnchecked: number) {
  const risks: string[] = [];
  ...
  if ((input.blast?.totalCallers ?? 0) >= 6) {
    risks.push(
      'Touched symbols have broad caller impact; regressions may appear outside the edited files.'
    );
  }
...
function inferRisks(fixture: CommitIntentFixture, totalChanged: number, uiFileCount: number) {
  ...
  if (fixture.author === 'agent' && uiFileCount > 0) {
    ...
    risks.push(
      'Agent-authored change; confirm it matches the intended task, not just a plausible diff.'
    );
  }
  if (uiFileCount > 0)
    risks.push('UI surface changed; screenshot or browser replay should exist before shipping.');
  if (totalChanged > 120)
    risks.push('Large diff for one intent; inspect for accidental refactor drift.');
  if (fixture.changedFiles.some((file) => file.surface === 'config'))
    risks.push('Config changed; verify deploy/build assumptions.');
