...
test.describe('Ask CodeVetter page', () => {
  ...
  test('/ask renders the chat surface', async ({ page }) => {
    ...
    await expect(page.locator('h1', { hasText: 'Ask CodeVetter' })).toBeVisible();
    await expect(page.getByPlaceholder('Which project shipped the most last week?')).toBeVisible();
