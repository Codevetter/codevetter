...
test.describe("Review page - Evidence", () => {
  ...
  test("can add and persist verification evidence to a finding", async ({ page }) => {
    ...
    const evidenceLevelSelect = page.locator("label", { hasText: "Evidence level" }).locator("select");
    const recheckStatusSelect = page.locator("label", { hasText: "Re-check status" }).locator("select");
    const artifactInput = page.locator("label", { hasText: "Artifact" }).locator("input");
    const notesTextarea = page.locator("label", { hasText: "QA steps / notes" }).locator("textarea");
