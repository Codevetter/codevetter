...
test.describe('Review page', () => {
  ...
  test('Right panel shows placeholder when no review is active', async ({ page }) => {
    // When no review is active, the right panel shows the static review preview.
    await expect(page.locator('text=review preview')).toBeVisible();
    await expect(page.locator('text=Select a repository and diff')).toBeVisible();
  });
  ...
  test('Clicking a past review shows findings with severity badges', async ({ page }) => {
    ...
    const hasResults = (await page.locator('h2', { hasText: 'Review Results' }).count()) > 0;
  ...
  test('Clicking New Review returns to the create form', async ({ page }) => {
    ...
    if (!hasSection) {
      ...
      await expect(page.locator('button', { hasText: 'Select repository...' })).toBeVisible();
      return;
    }
