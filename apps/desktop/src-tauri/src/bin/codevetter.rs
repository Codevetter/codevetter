...
fn render_human_ops(receipt: &OpsStatusReceipt) -> String {
    format!(
        ...
        if receipt.billing.anthropic_configured { "configured" } else { "not configured" },
        if receipt.billing.openai_configured { "configured" } else { "not configured" },
        if receipt.webhook.configured { "configured" } else { "not configured" },
        receipt.webhook.flavor,
...
mod tests {
    ...
    fn runs_parser_and_human_output_are_bounded() {
        ...
        let history = RunHistoryReceipt {
            ...
            returned: 1,
            runs: vec![
                codevetter_desktop::commands::run_history::RunHistoryRecord {
                    schema_version: "codevetter.run-record/v1".into(),
                    id: receipt.run_id.clone(),
                    kind: codevetter_desktop::commands::run_history::RunKind::LocalCheck,
    ...
    fn ops_parser_and_human_output_preserve_read_only_secret_boundary() {
        ...
        assert_eq!(arguments.output, OutputMode::Json);
        assert!(
            parse_arguments(["ops".into(), "--window-days".into(), "365".into()], cwd).is_err()
        );
        ...
        assert!(output.contains("Anthropic configured · OpenAI not configured"));
    ...
    fn settings_parser_preserves_one_explicit_non_secret_assignment() {
        ...
        assert_eq!(arguments.output, OutputMode::Json);
        assert!(
            parse_arguments(["settings".into(), "--set".into(), "missing".into()], cwd,).is_err()
        );
        assert!(parse_arguments(
            [
                "settings".into(),
    ...
    fn onboarding_parser_separates_inspection_from_explicit_completion() {
        let cwd = Path::new("/tmp/widget");
        assert!(matches!(
            parse_arguments(["onboarding".into(), "--json".into()], cwd)
                .expect("inspect onboarding"),
    ...
    fn performance_parser_and_human_output_preserve_the_closed_contract() {
        ...
        let receipt = PerformanceRunReceipt {
            ...
            stderr_summary: None,
            cleanup: codevetter_desktop::commands::performance_bridge::PerformanceCleanupReceipt {
