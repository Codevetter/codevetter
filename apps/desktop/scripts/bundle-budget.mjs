...
for (const c of chunks.slice(0, 12)) {
  ...
  console.log(`${c.name.padEnd(34)}${kb(c.raw).toFixed(1).padStart(10)}${kb(c.gz).toFixed(1).padStart(10)}${over}`);
}
if (chunks.length > 12) console.log(`… and ${chunks.length - 12} more chunks`);
console.log("-".repeat(54));
console.log(`${"TOTAL".padEnd(34)}${kb(totalRaw).toFixed(1).padStart(10)}${kb(totalGz).toFixed(1).padStart(10)}`);

const failures = [];
const biggest = chunks[0];
if (kb(biggest.raw) > PER_CHUNK_KB) {
  failures.push(`chunk ${biggest.name} is ${kb(biggest.raw).toFixed(0)} KB (budget ${PER_CHUNK_KB} KB)`);
}
