...
export async function prepareCollectorSidecars({
  target = process.env.TAURI_ENV_TARGET_TRIPLE ?? rustHostTarget(),
  binaryRoot = join(tauriRoot, 'resources/collectors'),
  resourceRoot = join(tauriRoot, 'resources/rustsec-advisory-db/snapshot'),
  cacheRoot = join(tauriRoot, 'target/codevetter-collector-archives'),
