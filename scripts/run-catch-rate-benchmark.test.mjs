...
test('redundant-match gate fails duplicate matches in one case', () => {
  const fixture = readSample();
  fixture.cases[0].reviews.codevetter[1].matched_ground_truth = ['missing-empty-state-action'];
...
test('evidence comparison reports catch-rate deltas and per-case evidence impact', () => {
  ...
  assert.deepEqual(
    payload.evidence_comparison.cases.find((row) => row.id === 'agent-auth-boundary-001').regressed,
...
test('curation report flags generated TODO cases as incomplete', () => {
  ...
  assert.ok(
    report.rows[0].issues.some((issue) => issue.includes('TODO') || issue.includes('missing'))
  );
});
