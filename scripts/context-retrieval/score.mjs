...
export async function scoreRetrieval({
  ...
  for (const providerId of providerIds) {
    ...
    // Checked before EVERY provider, not once at startup. codesearch alone peaks at
    // 3.1 GB indexing a 168-file repo, so a run that was safe to begin can become
    // unsafe midway. Abort with partial results rather than take the machine down.
    ...
    const cases = [];
    // A provider that has failed to index the same repository several times running
    // will keep failing, and each failure costs a full call timeout. gortex burned
    // roughly 55 minutes proving hugo unindexable 11 times at 5 minutes apiece.
    // Abandon the arm instead, and record why so the skip is never mistaken for a
    // score. Consecutive rather than cumulative: a provider that recovers continues.
    let consecutiveHardFailures = 0;
    let abandoned = null;
    for (const [index, entry] of corpus.cases.entries()) {
      if (abandoned) break;
      // Deliberate mismatch: the query belongs to a different case, the revision
      // still belongs to this one. A provider that ignores the query scores the same.
      const source = shuffleQueries ? corpus.cases[(index + 1) % corpus.cases.length] : entry;
      // Awaited because MCP-backed providers are asynchronous; synchronous
      // adapters resolve immediately.
      ...
      // A capacity refusal is as hard as an index failure — retrying 29 times cannot
      // make a snapshot smaller than the ceiling that rejected it.
      const hard = /^(index|worktree)|exceeds?[- ].*limit|import[- ]limit|safety limit/i.test(
        response.unavailable_reason ?? ''
      );
      consecutiveHardFailures = hard ? consecutiveHardFailures + 1 : 0;
  ...
  // Outcome taxonomy per provider: never let did-not-install read as poor retrieval.
  ...
  return {
    ...
    limitations: [
      ...(gates.trustworthy
        ? []
        : [
            `RELIABILITY GATE FAILED — do not publish these numbers: ${[gates.controls_present.reason, gates.controls_lose.reason].filter(Boolean).join(' ')}`,
          ]),
      ...(aborted
        ? [
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === '--repomix') {
      options.repomix = true;
      continue;
    }
    if (argument === '--shuffle-queries') {
      options.shuffleQueries = true;
      continue;
    }
    if (
      ![
    ...
    index += 1;
    if (argument === '--corpus') options.corpusPath = value;
    if (argument === '--repo') options.repo = value;
    if (argument === '--provider') options.providerIds = value.split(',');
    if (argument === '--format') options.format = value;
    if (argument === '--out') options.out = value;
    if (argument === '--tool') options.tool = value;
    if (argument === '--cache-dir') options.cacheDir = value;
    if (argument === '--graphify') options.graphifyBinary = value;
    if (argument === '--codesearch') options.codesearchBinary = value;
    if (argument === '--codegrok') options.codegrokCommand = value;
    if (argument === '--serena') options.serenaCommand = value;
    if (argument === '--cocoindex') options.cocoindexCommand = value;
    if (argument === '--jcodemunch') options.jcodemunchCommand = value;
    if (argument === '--repomapper-python') options.repomapperPython = value;
    if (argument === '--repomapper-script') options.repomapperScript = value;
    if (argument === '--ripgrep') options.ripgrepBinary = value;
    if (argument === '--cli-tool') {
      const [id, bin] = value.split('=');
      (options.cliTools ??= {})[id] = bin;
    }
  }
