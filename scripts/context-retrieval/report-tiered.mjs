...
export function loadTiered(paths) {
  ...
  // A stored gate verdict describes the run that wrote it, and one run is not
  // necessarily one experiment. When each arm is measured in its own process — which is
  // how a 42-second-per-query tool is kept from gating twenty-four others — every
  // artifact truthfully reports "controls absent", because the controls are in sibling
  // files. Trusting the stored verdict therefore stamped "gate failed" on all 25 arms
  // of a run whose controls were present and losing. A verifier reading that either
  // rejects sound numbers or learns to ignore the gate, and the second is worse.
  //
  // So the gate is recomputed here, over the union of everything loaded, and the stored
  // verdict is honoured only when it fails for a reason the union cannot re-derive.
  const unionProviders = new Set();
  const unionScored = [];
  for (const path of paths) {
    try {
      ...
      for (const entry of score.providers ?? []) {
        unionProviders.add(entry.provider_id);
        if (entry.summary)
          unionScored.push({ provider_id: entry.provider_id, summary: entry.summary });
      }
    } catch {
      // Re-read below; a bad file fails loudly there.
    }
  }
  const unionControls = checkControlsPresent([...unionProviders]);
  const unionLose = checkControlsLose({ providers: unionScored });
  const unionOk = unionControls.ok && unionLose.ok !== false;
  for (const path of paths) {
    const score = JSON.parse(readFileSync(path, 'utf8'));
