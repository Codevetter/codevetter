# reference graph implementation parity contract

Last verified: 2026-07-14

CodeVetter uses reference graph implementation as the minimum useful code-graph reference, not as a
runtime dependency. The comparison is pinned to the MIT-licensed
`pinned external reference implementation` default `v8` branch at commit
`961b78e57a10e9c5bb98421ff3e45b40be73542b`. The checked fixture provenance is
recorded in
`apps/desktop/src-tauri/tests/fixtures/structural-parity-v1/UPSTREAM.md`.
...
| Interchange | Node-link `graph.json` plus HTML, report, wiki, database, and other optional exports | Bounded schema-validated reference graph implementation node-link import; versioned CodeVetter JSON and Markdown export; unknown fields retained as interchange extensions; imports never replace the canonical graph silently | Meets reference graph implementation import/local handoff; intentionally narrower export catalog | `structural_graph/interchange.rs`; interchange tests; adapter descriptors |
| Source evidence | Source file/line metadata on symbols and explained edges | Source anchors on nodes and edges, preserved through storage, queries, reference graph implementation import, Review context, proof export, and source-opening UI | Meets | `structural_graph/storage.rs`; `review.rs`; `review-proof.ts`; round-trip and Review trust tests |
...
| Local/offline core | Code extraction is local; optional document/media semantic passes and add-ons can use external services | Canonical build/query is bundled Rust, offline, non-mutating, and does not require reference graph implementation, Python, Node, an LLM, or network access | Meets and is stricter for the canonical path | adapter descriptors; secret-policy tests; offline runtime qualification |

## Measured fixture contract

The pinned fixture mirrors reference graph implementation’s cross-package Rust false-positive guard
...
| --- | ---: | ---: | ---: | ---: |
| Pinned reference graph implementation v8 fixtures | 3/3 | 3/3 | 0.0027 / 0.0037 ms | 0.0005 / 0.0005 ms |
...
- standalone `graph.html` and the full reference graph implementation export catalog;
- reference graph implementation’s full language breadth beyond CodeVetter’s documented 15-language set.
...
resolution, analysis, query, incremental edit/delete/rename, reference graph implementation import,
