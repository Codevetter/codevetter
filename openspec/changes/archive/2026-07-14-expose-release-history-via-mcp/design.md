...
`add-graph-trust-paths` defines the canonical structural graph and reference graph implementation-grade query service; `add-release-history-graph` adds release checkpoints, commit deltas, as-of reconstruction, entity lineage, causal threads, annotations, evidence, and explanation facets. Agents should consume those contracts directly instead of scraping the UI, opening SQLite, or receiving full exported graphs/reports in every prompt.
