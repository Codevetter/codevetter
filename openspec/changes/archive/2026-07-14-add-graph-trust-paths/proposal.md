...
- Add incremental changed-file indexing, stale/deleted-node repair, deterministic IDs, graph snapshots, JSON import/export, and explicit reference graph implementation interoperability.
- Add graph intelligence at least equivalent to reference graph implementation’s useful code workflow: communities/subsystems, highest-degree hubs, bridge/cross-community relationships, surprising connections, neighborhood, impact, query, explain, and trust-weighted path.
...
- `structural-repo-graph`: Build, maintain, analyze, query, visualize, import, and export a reference graph implementation-grade local repository graph with trustworthy source evidence.
