...
- [x] 4.2 Add reference graph implementation node-link JSON import with engine/origin labeling and versioned CodeVetter JSON/Markdown export.
- [x] 4.3 Add optional reference graph implementation/GitNexus adapter boundaries without making either runtime required for the canonical graph.
...
- [x] 5.4 Benchmark query relevance and latency against pinned reference graph implementation fixtures and raw-search baselines, including a large-repository corpus.
...
- [x] 7.1 Maintain a pinned reference graph implementation capability matrix covering AST symbols, cross-file edges, trust, communities, hubs/bridges, incremental update, query/explain/path, visualization, and interchange.
...
- [x] 7.3 Runtime-verify supported multi-language repos, a large repo, incremental edit/delete/rename, reference graph implementation import, graph workbench, Review context, and offline operation.
...
- [x] 8.5 Run incremental repair, status, secret-policy, workbench, retention, reference graph implementation parity, and full production verification.
