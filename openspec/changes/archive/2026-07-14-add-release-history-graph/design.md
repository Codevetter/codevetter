...
The active `add-graph-trust-paths` change now defines a reference graph implementation-grade canonical structural graph. Release history uses its stable node identities, trust/source contract, snapshots, and query service while remaining a separate temporal graph.
