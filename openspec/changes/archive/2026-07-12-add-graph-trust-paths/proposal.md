...
CodeVetter can already build and visualize repo graphs, but its persisted graph contract flattens relationship trust and its UI cannot answer the verification-critical question “how does this changed thing reach that behavior?” The current Graphify project demonstrates a useful narrow pattern—source-backed edge provenance plus path/explain queries—that can strengthen CodeVetter’s review evidence without turning it into a generic knowledge-graph product.
...
- Restore explicit graph JSON import and normalize current Graphify `graph.json` output into CodeVetter’s local preview contract without mutating the target repo or requiring Graphify at runtime.
...
- Keep Graphify’s broad document/media ingestion, assistant hooks, wiki, hosted integrations, and semantic-learning surfaces out of scope.
