...
pub(in crate::commands::history_graph) fn prune_unreachable_history(
    ...
    drop(statement);
    // Publication already owns an atomic transaction. Standalone callers own
    // one here; nested callers must join it rather than issuing another BEGIN.
    let owned_transaction = connection
        .is_autocommit()
        .then(|| connection.unchecked_transaction())
        .transpose()
        .map_err(|error| format!("Start unreachable history cleanup: {error}"))?;
    let transaction = owned_transaction.as_deref().unwrap_or(connection);
    let mut removed = 0;
    ...
    if let Some(transaction) = owned_transaction {
        transaction
            .commit()
