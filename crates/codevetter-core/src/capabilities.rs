    ...
    let registry = CapabilityRegistry {
        ...
        capabilities: vec![
            ...
            capability(
                ...
                Qualification::Partial,
                &[
                    "Integration credentials remain outside the non-secret settings receipt until secure native storage is separately qualified.",
                    ...
                    "Ops read-only aggregate status now has a bounded shared contract; credential writes, live provider refresh, and webhook operations remain unavailable pending a secure native contract.",
            ...
            capability(
                ...
                &[
                    ...
                    "Credential and endpoint writes remain unavailable until a secure native contract is qualified.",
            ...
            capability(
                ...
                &[
                    ...
                    "Native UI and CLI share the exact persisted preference keys, defaults, and options.",
                    "The Evidence Workbench stores configuration only; it does not launch a helper or action live agent requests.",
                    "Agent and MCP surfaces receive no Agent Island authority.",
                ],
                "Treat a future live helper as a separately scoped side quest; the core verification product grants it no runtime authority.",
            ),
            capability(
            ...
            capability(
                ...
                Qualification::Partial,
                &[
                    "Rust-owned SQLite is the canonical rubric store; the installed-upgrade gate preserves an existing custom pack and active selection across native replacement and rollback.",
                    "Built-in packs are immutable; custom packs can be created or replaced within declared bounds.",
                ],
                "Keep the installed-upgrade custom-pack continuity gate in every production release qualification.",
            ),
            capability(
            ...
            capability(
                ...
                &[
                    tool("Gitleaks 8.30.1", "Scans the selected change without retaining raw secret values", "packaged product resource"),
                    tool("cargo-audit 0.22.2", "Checks Cargo.lock against a pinned offline RustSec snapshot", "packaged product resource"),
                    tool("cargo-llvm-cov 0.9.0", "Captures changed Rust line and region evidence for one explicit test target", "packaged product resource"),
                ],
                "Explicit change range in the selected local repository; collectors receive only their declared inputs.",
                Qualification::Partial,
                &["The native glossary reports declared collectors and limitations but does not execute them.", "Collectors never install tools, refresh the advisory database, or enable network access during verification.", "Rust coverage requires a user-selected test target and an existing llvm-tools-preview component.", "The resources are qualified in the local package path; signed and notarized release-bundle proof remains pending."],
                "Add bounded collector receipt inspection to the native UI and scoped MCP without granting either surface collector-execution authority, then repeat qualification in the signed production archive.",
            ),
            capability(
            ...
            capability(
                ...
                CapabilityStage::Current,
                surfaces(
                    projection(Availability::Available, Authority::ReadExecute, &["apps/macos"]),
                    projection(Availability::Unavailable, Authority::None, &[]),
                    projection(Availability::Unavailable, Authority::None, &[]),
                ),
                &[
                ...
                Qualification::Partial,
                &[
                    "The SwiftUI/AppKit client is the sole desktop application; no WebView application runtime is shipped.",
                    "Developer ID signing, notarization, Sparkle signing, installed upgrade, and rollback are fail-closed production release gates.",
                ],
                "Keep exact-package performance, accessibility, visual, installed-upgrade, rollback, and owner-acceptance receipts current for each production release.",
            ),
            capability(
                "evidence.local_runs",
