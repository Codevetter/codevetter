...
- **2026-07-07 (v1.2.11) — Repo workspace cleanup + Unpack usefulness pass:** merged the old Intel surface into Repo/Unpack as Activity, removed standalone Roadmap/resources from top-level navigation, cleaned the project sidebar, and made Unpack emphasize deterministic recommended next actions, graph-first repo memory, collapsible supporting evidence, past snapshots, and optional AI analysis on the same local snapshot.
